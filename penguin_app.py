...
flip_length = st.sidebar.slider("FLIPPER LENGTH", float(df["flipper_length_mm"].min()), float(df["flipper_length_mm"].max()))
...
if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    species_type = prediction(svc_model,island,bill_length,bill_depth,flip_length,body_mass,sex)
    score = svc_model.score(X_train, y_train)

  elif classifier =='Logistic Regression':
    species_type = prediction(log_reg,island,bill_length,bill_depth,flip_length,body_mass,sex )
    score = log_reg.score(X_train, y_train)

  else:
    species_type = prediction(rf_clf,island,bill_length,bill_depth,flip_length,body_mass,sex)
